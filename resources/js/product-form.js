document.addEventListener('DOMContentLoaded', function() {
    // Listen for form submission events
    window.addEventListener('formSubmitting', function() {
        // Find the submit button and add loading state
        const submitButtons = document.querySelectorAll('button[type="submit"]');
        submitButtons.forEach(function(button) {
            // Check if this is the create form submit button
            if (button.textContent.includes('Создать') || button.textContent.includes('Create')) {
                button.disabled = true;
                button.innerHTML = '<svg class="animate-spin -ml-1 mr-2 h-4 w-4 text-white inline" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg> Создание...';
            }
        });
    });

    window.addEventListener('formSubmitted', function() {
        // Find the submit button and remove loading state
        const submitButtons = document.querySelectorAll('button[type="submit"]');
        submitButtons.forEach(function(button) {
            // Check if this is the create form submit button
            if (button.textContent.includes('Создать') || button.textContent.includes('Create')) {
                button.disabled = false;
                button.innerHTML = 'Создать';
            }
        });
    });
});

// Also handle the case where the page is already loaded
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initializeProductForm);
} else {
    initializeProductForm();
}

function initializeProductForm() {
    // Additional initialization if needed
    console.log('Product form handler initialized');
}