...
Автоматический деплой на production сервер при merge в `main`.
...
#### `SSH_KEY` *(обязательный)*
Приватный SSH ключ для подключения к серверу. 

⚠️ **Важно:** Используйте формат OpenSSH:
```
...
#### `PORT` *(опционально)*
SSH порт сервера (по умолчанию 22). Используйте если у вас нестандартный порт:
```
2222
```
