...
class Product extends Model
{
    ...
    protected static function boot(): void
    {
        ...
        static::saving(function (Product $product) {
            // Автоматически поддерживаем volume_per_unit на сервере
            if ($product->calculated_volume !== null && (int) $product->quantity > 0) {
                $product->volume_per_unit = round((float) $product->calculated_volume / (int) $product->quantity, 4);
            } else {
